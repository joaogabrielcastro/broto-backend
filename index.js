...
const corsOptions = {
  origin: 'https://broto-frontend.vercel.app', 
...
// **********************************************
// ROTAS EXISTENTES (Caminhões e Viagens)
// **********************************************
...
// Rota para excluir caminhão
app.delete("/caminhoes/:id", async (req, res) => {
  const id = req.params.id;
  if (isNaN(parseInt(id))) {
    return res.status(400).json({ erro: "ID do caminhão inválido." });
  }
  try {
    const checkViagens = await db.query(`SELECT COUNT(*) FROM viagens WHERE caminhao_id = $1`, [id]);
    if (parseInt(checkViagens.rows[0].count) > 0) {
      return res.status(409).json({ erro: "Não é possível excluir o caminhão pois ele está associado a viagens." });
    }
    const result = await db.query(`DELETE FROM caminhoes WHERE id = $1`, [id]);
    if (result.rowCount === 0) {
      return res.status(404).json({ erro: "Caminhão não encontrado para exclusão." });
    }
    res.status(200).json({ excluido: true, id: id });
  } catch (err) {
    console.error("Erro no DB ao excluir caminhão:", err.message);
    res.status(500).json({ erro: "Erro interno do servidor ao excluir caminhão." });
  }
});
// NOVO: ROTA DELETE PARA VIAGENS
app.delete("/viagens/:id", async (req, res) => {
  ...
  try {
    const result = await db.query(`DELETE FROM viagens WHERE id = $1`, [id]);
    if (result.rowCount === 0) {
      return res.status(404).json({ erro: "Viagem não encontrada para exclusão." });
    }
    res.status(200).json({ excluido: true, id: id });
  } catch (err) {
    console.error("Erro no DB ao excluir viagem:", err.message);
    res.status(500).json({ erro: "Erro interno do servidor ao excluir viagem." });
  }
});
// 2. Cadastrar viagem
...
app.put("/viagens/:id", async (req, res) => {
  ...
  if (!destino || typeof destino !== 'string' || destino.trim() === '') {
    return res.status(400).json({ erro: "O destino da viagem é obrigatória para a edição." });
  }
  motorista_id = parseInt(motorista_id);
  cliente_id = parseInt(cliente_id);
  frete = parseFloat(frete);
